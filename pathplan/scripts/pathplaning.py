...
def XdirectionZigPath(xmin,xmax,ymin,ymax,Nx):
    ...
    for n in range(0,Nx):
        ...
        if flag==-2:
            p1 = ocl.Point(x,ymax,0)   # start-point of line
            p2 = ocl.Point(x,ymin,0)   # end-point of line
...
def vtk_visualize_parallel_finish_zig(stlfile, toolpaths):
	...
	for path in toolpaths:
		...
		if (first == True): # green sphere at path start
			...
			myscreen.addActor( camvtk.Line(p1=( pos.x,pos.y,pos.z),p2=( first_pt.x,first_pt.y,first_pt.z),color=XYrapidColor) )
